...
// Credits : https://github.com/improbable-eng/grpc-web/blob/master/client/

//Notes : grpc-web slightly differs from regular http2/grpc  spec: https://github.com/grpc/grpc/blob/master/doc/PROTOCOL-WEB.md
...
class GrpcUtils {
    ...
    private static chunkParaser = new ChunkParser()

    static frameRequest(bytes: Uint8Array): Uint8Array {
    ...
    /**
     *  grpc-web response contains both response and headers we need to do extra processing to get actual resp
     * @param respBytes 
     */
    static parseResponseBytes(respBytes: Uint8Array): Uint8Array | undefined {
        const chunks = this.chunkParaser.parse(respBytes)
        return chunks.find(c => c.chunkType === ChunkType.MESSAGE)?.data
    }
}
