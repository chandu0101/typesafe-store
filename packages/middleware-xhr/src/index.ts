...
export type XhrGlobalUrlOptions = {
  headers?: Record<string, string>;
  onError?: (resp: Response) => any;
};

export type XhrMiddlewareOptions = {
  urlOptions?: Record<string, () => XhrGlobalUrlOptions>;
};

export class XhrMiddlewareUtils {
  ...
  >(
    ...
    meta: HttpActionMeta,
    globalOptions?: XhrGlobalUrlOptions
  ) {
  ...
  static getGlobalUrlOptions(
    url: string,
    mOptions?: XhrMiddlewareOptions
  ): XhrGlobalUrlOptions | undefined {
    let result: XhrGlobalUrlOptions | undefined = undefined;
...
>(
  ...
  const url = XhrMiddlewareUtils.getUrl(fetchRequest.url);
  const globalUrlOptions = XhrMiddlewareUtils.getGlobalUrlOptions(
    url,
    moptions
  );
  const options = XhrMiddlewareUtils.getOptions(
    fetchRequest,
    fetchMeta,
    globalUrlOptions
  );
...
export default function createHhrMiddleware(
  options?: XhrMiddlewareOptions
): MiddleWare<any> {
  return (store: TypeSafeStore<any>) => (next: Dispatch<Action>) => (
    ...
    if (isHttpAction(rg, action)) {
      //
      return processXhrAction(store, action, rg, options);
    } else {
      return next(action);
    }
  };
